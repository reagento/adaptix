...
from .provider import TypeRequestChecker, ConstrainingProxyProvider, create_builtin_tr_checker
from ..core import (
    BaseFactory,
    Provider,
    PipeliningMixin,
...
class BuiltinFactory(BaseFactory[BuiltinSearchState], PipeliningMixin):
    ...
    def provide(self, s_state: BuiltinSearchState, request: Request[T]) -> T:
        ...
        for offset, item in enumerate(full_recipe[start_idx:], start_idx):
            ...
            try:
                attr_name = provider.cls_request_dispatching[request_cls]
            except KeyError:
                continue
        ...
        if isinstance(value, tuple):
            try:
                ...
                if isinstance(pred, TypeRequestChecker):
                    ...
                    tr_checker = create_builtin_tr_checker(pred)
