...
from dataclass_factory_30.facade import bound, name_mapping
...
from dataclass_factory_30.provider.name_layout import (
    BuiltinExtraMoveAndPoliciesMaker,
    BuiltinNameLayoutProvider,
    BuiltinSievesMaker,
    BuiltinStructureMaker,
)
from dataclass_factory_30.provider.request_cls import (
    DumperRequest,
    FieldLocation,
    LoaderRequest,
    Location,
...
def test_extra_at_list():
    ...
    with pytest.raises(
        ...
        match=full_match_regex_str(
            "Can not use collecting extra_in with list mapping"
            " at type <class 'tests_30.provider.name_layout.test_provider.Foo'>"
        )
    ):
        make_layouts(
            TestField('a'),
    ...
    with pytest.raises(
        ...
        match=full_match_regex_str(
            "Can not use collecting extra_in with list mapping"
            " at type <class 'tests_30.provider.name_layout.test_provider.Foo'> that situated at field 'foo'"
        )
    ):
        make_layouts(
            TestField('a'),
...
def test_required_field_skip():
    ...
    with pytest.raises(
        ValueError,
        match=full_match_regex_str(
            "Required fields ['a'] are skipped at type <class 'tests_30.provider.name_layout.test_provider.Foo'>"
        ),
    ):
        make_layouts(
            TestField('a', is_required=True),
...
def test_inconsistent_path_elements():
    ...
    with pytest.raises(
        ValueError,
        match=full_match_regex_str(
            "Inconsistent path elements at ('x',) at type <class 'tests_30.provider.name_layout.test_provider.Foo'>"
        ),
    ):
        make_layouts(
            TestField('a', is_required=True),
